...
namespace auto_rover_base
{
    ...
    double PID::operator()(const double &measured_value, const double &setpoint, const ros::Duration &dt)
    {
        ...
        // Reset the i_error in case the p_error and the setpoint are zero
    ...
    void PID::updateDynamicReconfig()
    {
        ...
        // Get starting values
        auto_rover_base::ParametersConfig config;
        config.f = f_;
    ...
    void PID::updateDynamicReconfig(auto_rover_base::ParametersConfig config)
    {
        // Make sure dynamic reconfigure is initialized
        if(!dynamic_reconfig_initialized_)
            return;
    ...
    void PID::dynamicReconfigCallback(auto_rover_base::ParametersConfig &config, uint32_t /*level*/)
    {

...
namespace auto_rover_dfrobot_gravity_10dof
{
    ...
    {
        public:
            explicit AutoRoverDFRobotGravity10DoF(const uint8_t bus) :
                fd_(-1), bus_(bus)
            {}
            ~AutoRoverDFRobotGravity10DoF();

            void SetConfiguration(DFRobotGravity10DoFConfig& config);
            int Connect(const uint16_t addr);
        ...
        private:
            ...
            uint8_t bus_;
            uint16_t addr_;
            uint8_t bmp_calib_[BMP280_CALIB_SIZE];
            double accel_bias_[BIAS_PARAM_SIZE];
